...
__global__ void kMeansClusterAssignment(ORB_SLAM2::GpuPoint *d_datapoints_, ORB_SLAM2::GpuPoint *final_points_, /*int *d_clust_assn_,*/ int2 *d_centroids_, uint *N_, int *K_, int maxLevel, int points_offset, int cluster_offset)
{
	...
	ORB_SLAM2::GpuPoint *d_datapoints = &(d_datapoints_[points_offset*level]);
	ORB_SLAM2::GpuPoint *final_points = &(final_points_[points_offset*level]);
...
// template <int T>
// __global__ void kMeansCentroidUpdate(ORB_SLAM2::GpuPoint *d_datapoints_, /*int *d_clust_assn_,*/ int2 *d_centroids_, int *d_clust_sizes_, uint *N_, int *K_, int maxLevel, int points_offset, int cluster_offset)
// {
// 	const int level = blockIdx.y;
...
// 	ORB_SLAM2::GpuPoint *d_datapoints = &(d_datapoints_[points_offset]);
...
__global__ void get_max_score(ORB_SLAM2::GpuPoint *d_datapoints_, uint *N_, int *k_scores_, int maxLevel, int points_offset, int cluster_offset){
	const int level = blockIdx.y;
	const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void get_max_points(ORB_SLAM2::GpuPoint *d_datapoints_, uint *N_, int *k_scores_, ORB_SLAM2::GpuPoint *final_points_, int maxLevel, int points_offset, int cluster_offset){
	const int level = blockIdx.y;
	const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void copy_points(ORB_SLAM2::GpuPoint *d_datapoints_, /*int *d_clust_assn_,*/ int *K_, uint *N_, ORB_SLAM2::GpuPoint *final_points_, int maxLevel, int points_offset){
	const int level = blockIdx.y;
	const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void last_cluster_assign(ORB_SLAM2::GpuPoint *d_datapoints_, ORB_SLAM2::GpuPoint *final_points_, int *k_scores_, /*int *d_clust_assn_,*/ int2 *d_centroids_, uint *N_, int *K_, int maxLevel, int points_offset, int cluster_offset)
{
	//get idx for this datapoint
	const int level = blockIdx.y;
	const int idx = blockIdx.x*blockDim.x + threadIdx.x;
