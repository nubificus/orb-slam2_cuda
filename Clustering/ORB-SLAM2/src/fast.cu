...
__global__
void fast_corner(uchar *images, uchar *inputImage, uint8_t *d_Rs, ORB_SLAM2::GpuPoint *buffers, int rows, int cols, uint8_t t_h, uint8_t t_low, uint *sizes, float *mvScaleFactor, int maxLevel, int inputImageStep){
    ...
    ORB_SLAM2::GpuPoint *buffer = &(buffers[level*cols*rows]);
...
__global__
void interpolate(const uint8_t *d_Rs, int cols, int rows, ORB_SLAM2::GpuPoint *buffers, uint *sizes, float *mvScaleFactor, int maxLevel){
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    const int level = blockIdx.z * blockDim.z + threadIdx.z;
    if (level >= maxLevel)
