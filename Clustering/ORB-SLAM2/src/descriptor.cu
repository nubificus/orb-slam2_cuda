...
__device__ inline void comp_descr(const uchar *image, ORB_SLAM2::GpuPoint &pt, cv::Point *pattern, int imageStep) {
...
__global__ void compute_descriptor_kernel(uchar *images, uchar *inputImage, ORB_SLAM2::GpuPoint *pointsTotal, const uint *sizes, cv::Point* pattern, int inputImageStep, int maxLevel, const float *mvScaleFactor, int cols, int rows) {
    ...
    ORB_SLAM2::GpuPoint *points = &(pointsTotal[level*cols*rows]);
